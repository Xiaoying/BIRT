...
public class StyleEntryWriter
{
	...
	private static final int[] PADDING_PROPS = {
		StyleConstant.PADDING_BOTTOM,
		StyleConstant.PADDING_TOP,
		StyleConstant.PADDING_LEFT,
		StyleConstant.PADDING_RIGHT				
	};
	
	private static final int[] BORDER_STYLE_PROPS = {
	...
	private static final int[] BORDER_COLOR_PROPS = {
	...
	private static final int[] BORDER_WIDTH_PROPS = {
			StyleConstant.BORDER_BOTTOM_WIDTH_PROP,
			StyleConstant.BORDER_TOP_WIDTH_PROP,
			StyleConstant.BORDER_LEFT_WIDTH_PROP,
			StyleConstant.BORDER_RIGHT_WIDTH_PROP				
	};
	
	private static final int[] MARGIN_PROPS = {
			StyleConstant.MARGIN_BOTTOM,
			StyleConstant.MARGIN_TOP,
			StyleConstant.MARGIN_LEFT,
			StyleConstant.MARGIN_RIGHT				
	};
	 ...
	 * @param noCellProps
	 */
	private void writeParagraphProperties( StyleEntry aStyle,
			boolean noCellProps )
	{
		// paragraph styles
		writer.openTag("style:paragraph-properties");
		// from writeTextInParagraph()		
		CSSValue lineHeight = (CSSValue)aStyle
				.getProperty( StyleConstant.LINE_HEIGHT );
	...
	protected void writeBorders( StyleEntry style, boolean includeMargins )
	{
		...
		{		
			...
			{
				writeSingleBorder( SIDES[i],
						borderStyle,
						style.getStringProperty( BORDER_COLOR_PROPS[i] ),
	...
	private void writeSingleBorder( String type, String borderStyle,
			String color, String width, String margin )	
	{
		writer.attribute( "fo:border-" + type,
				width + " " +
				borderStyle + " " +
				OdfUtil.parseColor( color )
				);
		if ( margin != null )
		{
	...
	private void writeLetterSpacing( StyleEntry style )
	{
		...
		if ( spacing != null )
		{
			double letterSpacing = OdfUtil.getDimensionValue( (CSSValue) style
					.getProperty( StyleConstant.LETTER_SPACING ), reportDpi );
	...
	private void writeCellProperties( StyleEntry style )
	{
		...
		writer.attribute( "fo:wrap-option", CSSConstants.CSS_NOWRAP_VALUE
				.equalsIgnoreCase( style
						.getStringProperty( StyleConstant.WHITE_SPACE ) )
				? "no-wrap"
				: "wrap" );
	}
	...
	private void writeBackgroundImage( StyleEntry style )
	{
		...
		if ( url != null )
		{
			...
			if ( "no-repeat".equals( repeat ) )
			{
				...
				writer.attribute( "style:position", 
						leftPos + " " + topPos );
			}
	...
	protected void writeFontSize( StyleEntry styleEntry )
	{
		...
		{		
			...
			if ( fontSize != null )
			{
				size = OdfUtil.parseFontSize( PropertyUtil
