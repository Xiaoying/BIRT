 ...
 * financial functions
 * 
 * @version $Revision: 1.4 $ $Date: 2010/11/01 01:21:22 $
 */
public class Finance
{
	 ...
	 * @param salvage
	 *            is the value at the end of the depreciation (sometimes called
	 *            the salvage value of the asset).
	 * @param life
	 *            is the number of periods over which the asset is being
	 *            depreciated (sometimes called the useful life of the asset).
	 * @param period
	 *            is the period for which you want to calculate the
	...
	public static double ddb( double cost, double salvage, double life,
			int period ) throws DataException
	{
	 ...
	 * @param pv
	 *            is the present value, or the lump-sum amount that a series of
	 *            future payments is worth right now. If pv is omitted, it is
	 *            assumed to be 0 (zero), and you must include the pmt argument.
	 * @param due
	 *            is the number 0 or 1 and indicates when payments are due. If
			...
			int due ) throws DataException
	{
		...
		/*
		 * Calculate the discount factor that gives you payment from present
		 * value
		 */
	 ...
	 *            type is omitted, it is assumed to be 0.
	 * @param per
	 *            is the period for which you want to find the interest and must
	 *            be in the range 1 to nper.
	 * @param fv
	...
	public static double ipmt( double rate, int per, int nPer, double pv,
			double fv, int due ) throws DataException
	{
		double pmt;
		double ipmt = 0;
		double principal;
			...
			int due, double guess ) throws DataException
	{
		try
		{
			...
			for ( i--; i > 0; i-- )
			{
				...
				{
					throw DataException.wrap( new AggrException( ResourceConstants.BAD_PARAM_COUNT,
							e ) );
				}
				if ( result != -1 )
					return result;
	 ...
	 * the examples below. If n is the number of cash flows in the list of
	...
	*/
	 ...
	 *            of cash flow values
	 * @param frate
	 *            is the interest rate you pay on the money used in the cash
	 *            flows.
	 * @param rrate
	 *            is the interest rate you receive on the cash flows as you
	 *            reinvest them.
